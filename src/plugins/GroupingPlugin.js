...
function resolveOptions(context) {
	...
	return {
		...
		order: 30,
		stateKey: 'grouping',
		label: context.getString('groupBy'),
		clearLabel: context.getString('noGrouping'),
		fields: [],
		defaultKey: '',
		control: 'select',
		multi: false,
		...
		dropdown: {
			summaryLabel: context.getString('grouping'),
			emptyLabel: context.getString('noGrouping'),
			headline: context.getString('groupRowsBy'),
			copy: context.getString('groupingHelp'),
			clearLabel: context.getString('clearGrouping'),
			preferredAlign: 'end',
			stateKey: '',
			wrapperClassName: '',
			detailsClassName: '',
